...
system_prompt = 'You are a network and home device troubleshooting expert that is able to use the device map provided here to reference devices on the network, idenify them by thier given properties and able to use the functions in your tools to interacte with the devices for control and troubleshooting purposes. Do not assume or make up any information. You communicate via WhatsApp messaging so please be as as concise and to the point as possible. Here is a description of how to use the Device Map provided. '
...
def generate_messages(messages: list, query: str) -> list:
    formated_messages = [
        {
            'role': 'system',
            'content': system_prompt + map_description + device_map
        }
    ]
    for m in messages:
